...
class PaginationServiceTest extends TestCase
{
    ...
    {
        $this->assertSame('/tech/', $this->service->pageUrl('/tech', 1));
    }

    public function testPageUrlForSecondPage(): void
    {
        $this->assertSame('/tech/page/2/', $this->service->pageUrl('/tech', 2));
    }

    public function testPageUrlTrailingSlashIdempotency(): void
    {
        $this->assertSame(
            $this->service->pageUrl('/tech', 2),
            $this->service->pageUrl('/tech/', 2)
        );
    }
    ...
    {
        $pagination = $this->service->buildPagination(1, 3, '/tech/');

        $this->assertNull($pagination->prevUrl);
        $this->assertSame('/tech/page/2/', $pagination->nextUrl);
    ...
    {
        $pagination = $this->service->buildPagination(2, 3, '/tech/');

        $this->assertSame('/tech/', $pagination->prevUrl);
        $this->assertSame('/tech/page/3/', $pagination->nextUrl);
    }

    public function testBuildPaginationLastOfThree(): void
    {
        $pagination = $this->service->buildPagination(3, 3, '/tech/');

        $this->assertSame('/tech/page/2/', $pagination->prevUrl);
        $this->assertNull($pagination->nextUrl);
    }

    public function testBuildPaginationSinglePage(): void
    {
        $pagination = $this->service->buildPagination(1, 1, '/tech/');
